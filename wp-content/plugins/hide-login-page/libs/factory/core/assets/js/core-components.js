...
(function($) {
	...
	var externalAddon = {
		...
		events: function() {
			...
			$(document).on('click', '.wfactory-473-process-button', function() {
				var $this = $(this),
				...
				var action = ('creativemotion' === storage)
				             ? 'wfactory-473-creativemotion-install-plugin'
				             : 'install-plugin';
				...
				if( storage === 'freemius' || ((storage === 'wordpress' || storage === 'creativemotion' || storage === 'internal') && (plugin_action === 'activate' || plugin_action === 'deactivate')) ) {
					action = 'wfactory-473-intall-component';
				...
				$.wfactory_473.hooks.run('core/components/pre_update', [$this, data]);

				self.sendRequest(data, function(response) {
					if( !response || !response.success ) {
						$.wfactory_473.hooks.run('core/components/update_error', [
							$this,
							data,
							response
						]);
					...
					if( response.success ) {
						...
						if( plugin_action === 'install' ) {
							...
							$.wfactory_473.hooks.run('core/components/installed', [
								$this,
								data,
								response
							]);
							...
							$.wfactory_473.hooks.run('core/components/pre_activate', [
								$this,
								data,
								response
							]);
							...
							$.wfactory_473.hooks.run('core/components/deactivated', [
								$this,
								data,
								response
							]);
							...
							$.wfactory_473.hooks.run('core/components/deleted', [$this, data, response]);
						}
					} else {
						if( plugin_action === 'install' ) {
					...
					$.wfactory_473.hooks.run('core/components/updated', [$this, data, response]);
				});
		...
		/**
		 * РћС‚РїСЂР°РІР»СЏРµС‚ РґРѕРїРѕР»РЅРёС‚РµР»СЊРЅС‹Р№ Р·Р°РїСЂРѕСЃ РЅР° Р°РєС‚РёРІР°С†РёСЋ РєРѕРјРїРѕРЅРµРЅС‚Р°, РІРѕ РІСЂРµРјСЏ Р°РєС‚РёРІР°С†РёРё
		 * РІС‹РїРѕР»РЅСЏРµС‚ С…СѓРє wbcr/clearfy/activated_component.
		 *
		 * Р’ РїСЂРёРЅС†РёРїРµ, СЌС‚Рѕ РЅРµРѕР±С…РѕРґРёРјРѕ РґР»СЏ РїРѕРґРіРѕС‚РѕРІРєРё РїР»Р°РіРёРЅР° Рє СЂР°Р±РѕС‚Рµ, Р·Р°РїРёСЃРё РЅРµРѕР±С…РѕРґРёРјС‹С… СЃС‚СЂРѕРє Рё С‚Р°Р±Р»РёС† РІ
		 * Р±Р°Р·Сѓ РґР°РЅРЅС‹С…, РїРµСЂРµРїРёСЃСЊ РїРѕСЃС‚РѕСЏРЅРЅС‹С… СЃСЃС‹Р»РѕРє, РїСЂРѕРІРµСЂРєР° РєРѕРЅС„Р»РёРєС‚РѕРІ Рё С‚.Рґ.
		 *
		...
		sendRequestToComponentActivationPrepare: function(componentButton, sendData, button_i18n) {
			...
			sendData.action = 'wfactory-473-prepare-component';
			...
			this.sendRequest(sendData, function(response) {
				...
				if( !response || !response.success ) {
					...
					$.wfactory_473.hooks.run('core/components/activation_error', [
						componentButton,
						sendData,
						response
					]);
				...
				$.wfactory_473.hooks.run('core/components/activated', [
					componentButton,
					sendData,
					response
				]);
		...
		sendRequest: function(data, callback) {
			...
			$.ajax(ajaxurl, {
				...
				error: function(xhr, ajaxOptions, thrownError) {
					...
					$.wfactory_473.hooks.run('core/components/ajax_error', [
						xhr,
						ajaxOptions,
						thrownError
					]);
